@extends('layouts.app')
@section('title', 'Prenota una Sessione | LaFamiglia808')
        ...
        <div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-4 p-4" style="background:#111; border:1px solid #1A1A1A;">
                    ...
                    <div>
                        <div class="font-mono text-xs tracking-widest uppercase mb-1" style="color:#444;">{{ $info[1] }}</div>
                        <div class="text-sm text-white">{{ $info[2] }}</div>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <form action="{{ route('booking.store') }}" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="time" required
                            ...
                            @foreach(['10:00','11:00','12:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00','21:00'] as $t)
                ...
                <div>
                    ...
                    <select name="duration"
                            ...
                            style="background:#0A0A0A;border:1px solid #2A2A2A;font-family:'Space Grotesk',sans-serif;outline:none;"
                            onfocus="this.style.borderColor='#DC2626'" onblur="this.style.borderColor='#2A2A2A'">
                        <option value="60" {{ old('duration','60')=='60'?'selected':'' }}>1 ora</option>
                        <option value="120" {{ old('duration')=='120'?'selected':'' }}>2 ore</option>
                        <option value="180" {{ old('duration')=='180'?'selected':'' }}>3 ore</option>
                        <option value="240" {{ old('duration')=='240'?'selected':'' }}>4 ore (mezza giornata)</option>
                        <option value="480" {{ old('duration')=='480'?'selected':'' }}>8 ore (giornata intera)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                              ...
                              style="background:#0A0A0A;border:1px solid #2A2A2A;font-family:'Space Grotesk',sans-serif;outline:none;"
                              onfocus="this.style.borderColor='#DC2626'" onblur="this.style.borderColor='#2A2A2A'"
                              placeholder="Raccontaci il tuo progetto, strumenti che porti, genere musicale...">{{ old('notes') }}</textarea>
                </div>
