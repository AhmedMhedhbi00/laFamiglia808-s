...
<head>
    ...
    <title>@yield('title', 'Admin') | LaFamiglia808</title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-60 flex-shrink-0 flex flex-col" style="background:#0D0D0D; border-right:1px solid #1A1A1A;">
        ...
        <nav class="flex-1 py-4 overflow-y-auto">
            ...
            <a href="{{ route('admin.messages.index') }}" class="sidebar-link {{ request()->routeIs('admin.messages*') ? 'active' : '' }}">
                ...
                Messaggi
                @php $unread = \App\Models\Message::where('read',false)->count(); @endphp
                @if($unread > 0)
                <span class="ml-auto text-xs px-2 py-0.5" style="background:#DC2626; color:#fff; border-radius:2px; font-family:'JetBrains Mono',monospace;">{{ $unread }}</span>
                @endif
            </a>
            <a href="{{ route('admin.bookings.index') }}" class="sidebar-link {{ request()->routeIs('admin.bookings*') ? 'active' : '' }}">
                ...
                @if($pending > 0)
                <span class="ml-auto text-xs px-2 py-0.5" style="background:#DC2626; color:#fff; border-radius:2px; font-family:'JetBrains Mono',monospace;">{{ $pending }}</span>
                @endif
            </a>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="flex items-center justify-between px-8 py-4 flex-shrink-0" style="background:#0D0D0D; border-bottom:1px solid #1A1A1A;">
            <h1 class="font-mono text-xs tracking-widest uppercase" style="color:#888;">@yield('page-title', 'Dashboard')</h1>
            <span class="font-mono text-xs" style="color:#333;">{{ session('admin_email') }}</span>
        </header>
